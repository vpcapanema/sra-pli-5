...
def _obter_dominio_id(tipo: str, valor: str | None = None) -> int:
    """Busca domínio obrigatório para os testes."""
    query = Dominio.query.filter_by(tipo=tipo)
...
def _criar_relatorio_test(
    *,
    ...
    """Cria RelatorioProducao sem kwargs dinâmicos para satisfazer Pyright."""
    relatorio = RelatorioProducao()
    relatorio.codigo_d20 = codigo_d20
    relatorio.numero_medicao = numero_medicao
    relatorio.mes_referencia = mes_referencia
    relatorio.periodo_inicio = periodo_inicio
    relatorio.periodo_fim = periodo_fim
    relatorio.titulo_curto = titulo_curto
    relatorio.status_id = status_id
    relatorio.criado_por = criado_por
    relatorio.versao_atual = versao_atual
    relatorio.caminho_template = caminho_template
    return relatorio
...
def _criar_usuario_test(app, suffix=''):
    ...
    usuario = Usuario(
        ...
        senha_hash='x',
        perfil_id=_obter_dominio_id('perfil_usuario', 'coordenador'),
        ativo=True,
    )
...
def test_property_4_sync_estrutura_basica(app):
    ...
    **Validates: Requirements 4.1**
    ...
    with app.app_context():
        ...
        status_id = _obter_dominio_id('status_relatorio')

        docx_path = _criar_docx_simples(['Introdução', 'Metodologia'])

        rel = _criar_relatorio_test(
...
def test_property_4_sync_classifica_multiplos_tipos(app, lista_capitulos: list):
    ...
    **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5**
    ...
    with app.app_context():
        ...
        assert 'capitulos_sincronizados' in resultado, (
            "Resultado deve ter 'capitulos_sincronizados'"
        )
...
def test_property_4_sync_idempotente_deterministica(
    app,
    num_ciclos: int,
    capitulos: list,
):
        ...
        for estilo_esperado, classif_esperada in estilos:
            ...
            if matching_caps:
                ...
                if classif_esperada is None:
                    ...
                    assert (
                        cap.classificacao == classif_esperada
                        or cap.classificacao is None
                    ), (
                        f"Estilo '{estilo_esperado}' deve ter classificacao "
                        f"'{classif_esperada}', obteve '{cap.classificacao}'"
                    )
...
def test_property_4_anexos_vs_apendices_distintos(
    app,
    num_anexos: int,
    num_apendices: int,
):
    ...
    **Validates: Requirements 4.2**
    ...
    with app.app_context():
        ...
        ressincronizar_capitulos_com_classificacao(rel)
        caps = CapituloDocumento.query.filter_by(id_relatorio=rel.id).all()
        ...
        assert isinstance(apendices, list)

        # Um capítulo não pode ser ao mesmo tempo anexo e apêndice.
        for cap in caps:
...
def test_property_4_robustez_titulos_variados(
    app,
    tamanho_titulo: int,
    com_acentos: bool,
    com_espacos: bool,
    ...
    **Validates: Requirements 4.1, 4.3**
    ...
    with app.app_context():
        ...
        try:
            resultado = ressincronizar_capitulos_com_classificacao(rel)
            assert isinstance(resultado, dict), (
                "Resultado deve ser dict mesmo com títulos variados"
            )
