...
class ServicoExtracaoSecoes:
    ...
    def extrair_secoes_do_docx(
        docx_path: str, id_relatorio: int
    ) -> List[SecaoDOCX]:
        ...
        try:
            ...
            return ServicoExtracaoSecoes._extrair_secoes_fallback(
                docx_path, id_relatorio
            )
        ...
        try:
            ...
            for i, section in enumerate(doc.sections):
                ...
                secao = SecaoDOCX(  # type: ignore[call-arg]
                    **{
                        'id_relatorio': id_relatorio,
                        'ordem_secao': i,
                        'tipo_secao': (
                            ServicoExtracaoSecoes._determinar_tipo_secao(
                                section
                            )
                        ),
                        'reiniciar_numero_pagina': (
                            section.start_type != 'continuous'
                        ),
                        'orientacao': orientacao,
                        'colunas': (
                            ServicoExtracaoSecoes._contar_colunas(section)
                        ),
                        'margem_superior': (
                        ...
                        'propriedades_raw': (
                            ServicoExtracaoSecoes._extrair_propriedades_raw(
                                section
                            )
                        ),
                    }
    ...
    def _extrair_secoes_fallback(
        ...
        secoes = [
            SecaoDOCX(  # type: ignore[call-arg]
                **{
                    'id_relatorio': id_relatorio,
                    'ordem_secao': 0,
                    'tipo_secao': 'nextPage',
    ...
    def extrair_quebras_pagina_do_docx(
        docx_path: str, secoes: List[SecaoDOCX]
    ) -> List[QuebraPagina]:
        ...
        try:
            ...
            for paragraph in doc.paragraphs:
                ...
                if hasattr(paragraph, 'runs'):
                    for run in paragraph.runs:
                        ...
                        if run.text and '\\page' in run.text:
                            quebra = QuebraPagina(  # type: ignore[call-arg]
                                **{
                                    'id_secao': (
                ...
                if (
                    paragraph.style
                    and paragraph.style.name in ['Heading 1', 'Título 1']
                ):
    ...
    def mapear_capitulos_para_secoes(
        ...
        for i, cap in enumerate(capitulos):
            ...
            if cap.tipo_elemento == 'textual' and not cap.classificacao:
                cap.id_secao_inicio = (
                    secoes[0].id_secao if secoes else 1
                )
            elif cap.classificacao in ('anexo', 'apendice'):
                cap.id_secao_inicio = (
                    secoes[1].id_secao
                    if len(secoes) > 1
                    else secoes[0].id_secao
                )
            ...
            if i < len(capitulos) - 1:
                ...
                cap.id_secao_fim = (
                    secoes[-1].id_secao if secoes else cap.id_secao_inicio
                )
    ...
    def analisar_estrutura_documento(
        ...
        analise = {
            ...
            'secoes_com_numero_diferente': sum(
                1 for s in secoes if s.tem_numero_pagina_diferente
            ),
            'secoes_com_orientacao_diferente': sum(
                1 for s in secoes if s.orientacao != 'portrait'
            ),
            'quebras_visiveis': sum(
                1 for q in quebras if q.e_quebra_visivel
            ),
            'estrutura_secoes': [
                {
                    'ordem': s.ordem_secao,
    ...
    def validar_mapeamento_secoes_capitulos(
        ...
        for cap in capitulos:
            if not cap.id_secao_inicio:
                erros.append(
                    f"Capítulo '{cap.titulo_capitulo}' "
                    f"não tem seção de início"
                )
            ...
            secao_inicio = next(
                (
                    s
                    for s in secoes
                    if s.id_secao == cap.id_secao_inicio
                ),
                None,
            )
            if not secao_inicio:
                erros.append(
                    f"Capítulo '{cap.titulo_capitulo}' "
                    f"referencia seção inexistente: {cap.id_secao_inicio}"
                )

            # Verificar se capítulos textuais não começam em seção de anexos
            if (
                cap.tipo_elemento == 'textual'
                and secao_inicio
                and secao_inicio.ordem_secao > 0
                and any(
                    c.classificacao in ('anexo', 'apendice')
                    for c in capitulos
                    if c.id_secao_inicio == secao_inicio.id_secao
                )
            ):
                erros.append(
                    f"Capítulo textual '{cap.titulo_capitulo}' "
                    f"começa em seção de anexos/apêndices"
                )
