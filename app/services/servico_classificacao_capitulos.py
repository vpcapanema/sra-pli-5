...
class ServicoClassificacaoCapitulos:
    ...
    ESTILOS_PARA_CLASSIFICACAO = {
        # Capítulos (nível 1)
        'capitulo': [
            'Heading 1', 'Título 1', 'Titulo 1', 'TÍTULO 1',
            'Titulo1', 'Heading1', 'Title 1', 'Titulo 1º'
        ],
        
        # Subcapítulos nível 2
        'subcapitulo_nivel_2': [
            'Heading 2', 'Título 2', 'Titulo 2', 'TÍTULO 2',
            'Titulo2', 'Heading2', 'Title 2', 'Titulo 2º'
        ],
        
        # Subcapítulos nível 3
        'subcapitulo_nivel_3': [
            'Heading 3', 'Título 3', 'Titulo 3', 'TÍTULO 3',
            'Titulo3', 'Heading3', 'Title 3', 'Titulo 3º'
        ],
        
        # Anexos
        'anexo': [
            'Anexo', 'ANEXO', 'Anexo A', 'Anexo_A',
            'Anexo Heading', 'Anexo Title', 'Anexo 1'
        ],
        
        # Apêndices
        'apendice': [
            'Apêndice', 'APÊNDICE', 'Apêndice I', 'Apêndice_I',
            'Apêndice Heading', 'Apêndice Title', 'Apêndice 1'
        ],
    ...
    def classificar_por_titulo(titulo: str, indice: str, nivel: int) -> Tuple[Optional[str], Optional[str]]:
        ...
        if ('anexo' in titulo_lower or 
            indice_upper.startswith('ANEXO') or
            (indice_upper and indice_upper[0] in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' and len(indice_upper) == 1)):
            return 'anexo', 'ANEXO_'
        
        # Verificar se é apêndice
        if ('apendice' in titulo_lower or 
            'apêndice' in titulo_lower or
            indice_upper.startswith('APENDICE') or
            indice_upper in ('I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X')):
    ...
    def determinar_tipo_elemento(titulo: str, posicao_relativa: int, total_secoes: int) -> str:
        ...
        if posicao_relativa > total_secoes * 0.7:
            ...
            pos_textuais = [
                'referências', 'referencias', 'bibliografia',
                'anexo', 'apêndice', 'apendice', 'glossário', 'glossario',
                'índice', 'indice'
            ]
    ...
    @staticmethod
    def gerar_indice_hierarquico(nivel: int, indice_pai: str = None, 
                                 sequencia: int = 1) -> str:
    ...
    def normalizar_capitulo(capitulo: CapituloDocumento) -> CapituloDocumento:
        ...
        if not capitulo.classificacao:
            classificacao, prefixo = ServicoClassificacaoCapitulos.classificar_por_titulo(
                ...
                capitulo.indice_capitulo,
                capitulo.nivel_capitulo
            )
            if classificacao:
    ...
    def criar_arvore_capitulos(capitulos: List[CapituloDocumento]) -> List[Dict]:
        ...
        """
        # Organizar por ID para acesso rápido
        capitulos_por_id = {cap.id_capitulo_documento: cap for cap in capitulos}
    ...
    @staticmethod
    def atualizar_indices_apos_operacao(capitulos: List[CapituloDocumento], 
                                        tipo_operacao: str,
                                        capitulo_afetado: Optional[CapituloDocumento] = None) -> List[CapituloDocumento]:
        ...
        for cap in capitulos_textuais:
            if cap.nivel_capitulo >= 2 and cap.capitulo_pai:
                ...
                subcapitulos = [c for c in capitulos_textuais 
    ...
    @staticmethod
    def extrair_e_classificar_do_docx(paragrafo_texto: str, estilo_docx: str, 
                                      posicao: int, total_paragrafos: int) -> Dict:
