...
class ServicoClassificacaoCapitulos:
    ...
    def classificar_por_estilo_docx(
        ...
        mapeamento = {
            'capitulo': (None, 1, None),
            'subcapitulo_nivel_2': (None, 2, None),
            'subcapitulo_nivel_3': (None, 3, None),
            'anexo': ('anexo', 1, 'ANEXO_'),
            'apendice': ('apendice', 1, 'APENDICE_'),
            'pre_textual': (None, 1, None),
            'pos_textual': (None, 1, None),
        }
        ...
        if 'apendice' in estilo_lower or 'apêndice' in estilo_lower:
            return 'apendice', 1, 'APENDICE_'
        for i in range(1, 4):
            if f'heading {i}' in estilo_lower or f'título {i}' in estilo_lower:
                return None, i, None
    ...
    def extrair_indice_puro(indice_com_prefixo: str) -> str:
        ...
        if indice.startswith('APENDICE_'):
    ...
    def criar_arvore_capitulos(capitulos: List[CapituloDocumento]) -> List[Dict]:
        ...
        raizes = [cap for cap in capitulos if cap.id_capitulo_pai is None]
        ...
        def construir_no(capitulo: CapituloDocumento) -> Dict:
            ...
            filhos = [
                ...
                if cap.id_capitulo_pai == capitulo.id_capitulo_documento
            ]
    ...
    def atualizar_indices_apos_operacao(
        ...
        capitulos_textuais = [
            ...
            if c.tipo_elemento == 'textual' and not c.classificacao
        ]
    ...
    def determinar_estilo_por_tipo_conceitual(
        ...
        if tipo_conceitual in mapeamento:
            return mapeamento[tipo_conceitual]
        if tipo_conceitual == 'subcapitulo':
            return {2: 'Heading 2', 3: 'Heading 3'}.get(nivel, f'Heading {nivel}')
        return None
