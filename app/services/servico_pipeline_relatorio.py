...
from app.services.servico_merge_docx import ServicioMergeDocx
from app.services.servico_captioning import ServicoCaptioning
from app.services.servico_cross_refs import ServicoCrossRefs
from app.services.servico_toc import ServicoToc
        ...
        try:
            ...
            if capitulos_nao_sync:
                ...
                titulos_nao_sync = ', '.join([
                    f"'{cap.titulo}' (ID {cap.id})"
                    for cap in capitulos_nao_sync[:3]  # Mostrar até 3
                ])
            ...
            if not relatorio.caminho_template:
                resultado['valido'] = False
                resultado['motivos_rejeicao'].append(
                    'Relatório não tem template DOCX configurado.'
                )
    ...
    def _fazer_merge(
        ...
        try:
            ...
            if resultado['capítulos_processados'] > 0:
                ...
                resultado['avisos'].append(
                    f'Documento salvo com {resultado["capítulos_processados"]} capítulos atualizados'
                )
            else:
