...
class QuebraPagina(db.Model, AuditoriaMixin):
    ...
    - SEÇÃO (w:sectPr): Define propriedades de formatação
    - QUEBRA DE PÁGINA (w:br): Força nova página/coluna dentro de uma seção
    - CAPÍTULO: Unidade conceitual de conteúdo
    
    Uma quebra de página é sempre dentro de uma seção e não afeta as propriedades da seção.
    """
    ...
    # Posição dentro da seção (em caracteres ou parágrafos)
    posicao_na_secao = db.Column(db.Integer, nullable=False)
    tipo_posicao = db.Column(
        db.String(20),
        default='paragrafo'
    )  # 'paragrafo', 'caractere', 'run'
    
    # Tipo de quebra (conforme OOXML w:type)
    tipo_quebra = db.Column(
        db.String(50),
        nullable=False,
        default='page'
    )  # 'page', 'column', 'textWrapping'
    
    # Propriedades específicas
    forcar_nova_pagina = db.Column(db.Boolean, default=True)
    limpar_formatacao = db.Column(db.Boolean, default=False)
    
    # Contexto da quebra (opcional)
    contexto_anterior = db.Column(db.Text, nullable=True)  # Texto antes da quebra
    contexto_posterior = db.Column(db.Text, nullable=True)  # Texto depois da quebra
    ...
    def posicao_relativa(self):
        ...
        elif self.tipo_posicao == 'caractere':
