...
class RelatorioProducao(db.Model):
    ...
    modelo_id = db.Column(
        db.Integer, db.ForeignKey("modelos_relatorio.id_modelo_relatorio"), nullable=True
    )
    ...
    status_id = db.Column(db.Integer, db.ForeignKey("dominios.id_dominio"), nullable=False)
    criado_por = db.Column(db.Integer, db.ForeignKey("usuarios.id"), nullable=False)
    criado_em = db.Column(db.DateTime, default=db.func.now(), nullable=False)
    atualizado_em = db.Column(db.DateTime, nullable=True)
    ano_referencia = db.Column(db.Integer, nullable=True)
    versao_atual = db.Column(db.String(20), nullable=False, default="R00")
    bloqueio_edicao = db.Column(db.Boolean, default=False, nullable=False)
    ...
    biblioteca = db.relationship("BibliotecaFormatacaoCanonica", foreign_keys=[biblioteca_id])
    capitulos = db.relationship("CapituloDocumento", back_populates="relatorio")
    envios = db.relationship("EnvioConteudo", back_populates="relatorio")
    revisoes = db.relationship("Revisao", back_populates="relatorio")
    bloqueios = db.relationship("Bloqueio", back_populates="relatorio")
    finalizacoes = db.relationship("RelatorioFinalizado", back_populates="relatorio_producao")
    secoes = db.relationship(
        "SecaoDOCX", back_populates="relatorio", order_by="SecaoDOCX.ordem_secao"
    )
