...
class RelatorioFinalizado(db.Model):
    ...
    relatorio_id = db.Column(db.Integer, db.ForeignKey("relatorios_producao.id"), nullable=True)
    modelo_id = db.Column(
    ...
    finalizado_por = db.Column(db.Integer, db.ForeignKey("usuarios.id"), nullable=False)
    data_finalizacao = db.Column(db.DateTime, default=db.func.now(), nullable=False)
    ...
    versao = db.Column(db.String(20), nullable=False, default="R00")
    sincronizado_em = db.Column(db.DateTime, default=db.func.now(), nullable=False)

    relatorio_producao = db.relationship("RelatorioProducao", back_populates="finalizacoes")
    finalizador = db.relationship("Usuario", foreign_keys=[finalizado_por])
