...
from flask import Blueprint, redirect, url_for, flash, request, session, render_template, jsonify
...
def listar_usuarios():
    ...
    return render_conteudo(["admin/gestao_usuarios.html"], usuarios=usuarios)
...
def criar_usuario():
    ...
    try:
        ...
        if request.headers.get("Accept") == "application/json":
            return jsonify({"erro": str(erro), "logs": logs}), 400
        flash(str(erro), "erro")
    return redirect(url_for("admin.listar_usuarios"))


@admin_bp.route("/usuarios/<int:id_usuario>/reenviar", methods=["POST"])
...
@admin_bp.route("/usuarios/<int:id_usuario>/ativar", methods=["POST"])
